...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 🌌 ACETERNITY HOVER ENGINE (Unified Interaction)
    initAceternityHover('.hero-proof', '.proof-item');
    initAceternityHover('.stats-grid', '.stat-card');
    initAceternityHover('.industries-grid', '.industry-card');
    initAceternityHover('.skills-grid', '.skill-chip');
    initAceternityHover('.kinetic-list', '.kinetic-item');
});

/**
 * ACETERNITY HOVER FOLLOW ENGINE
 * Replicates the floating backdrop effect from Aceternity UI.
 */
function initAceternityHover(containerSelector, itemSelector) {
    const containers = document.querySelectorAll(containerSelector);

    containers.forEach(container => {
        // Create the backdrop element
        const backdrop = document.createElement('div');
        backdrop.className = 'hover-backdrop-follower';
...
/**
 * ULTRA 2026 - ARCHITECTURAL MOTION
 * Combination of Blueprint Reveal (1) + Text Scramble (4)
 * Exceptional Performance Edition
 */
...
class ArchitectMotion {
    constructor() {
        ...
        if (this.eyebrow) {
            ...
            this.eyebrow.style.visibility = 'hidden';
        }
        if (this.tagline) {
            this.tagline.style.opacity = '0';
            this.tagline.style.visibility = 'hidden';
        }
    ...
    init() {
        setTimeout(() => {
            if (this.eyebrow) this.animate(this.eyebrow, 1200, 0);
            if (this.tagline) this.animate(this.tagline, 2000, 400);
        }, 300);
    }

    animate(el, duration, delay) {
        const originalText = el.innerText;
        el.innerText = '';
        el.style.opacity = '1';
        el.style.visibility = 'visible';
        el.classList.add('blueprint-text');
        ...
        const step = (timestamp) => {
            ...
            for (let i = 0; i < originalText.length; i++) {
                if (i < revealedChars) {
                    ...
                    // Show a few scrambling characters ahead of the revealed ones
            ...
            if (progress < 1) {
                ...
                el.classList.add('architect-glow');
            }
        };
